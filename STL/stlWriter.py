...
class ASCII_STL_Writer:
    ...
    def add_face(self, face):
        """ Add one face with 3 or more vertices. """
        #if len(face) == 4:
        ...
        if len(face) == 3:
            ...
            faceList = self._split(face)
            for fc in faceList:
              self._write(fc)
        else:
            #raise ValueError('only 3 or 4 vertices for each face')
            raise ValueError('3 or more vertices for each face')
    ...
    def add_triangle_faces(self, triangles,points):
        """ Add many faces. """
        for tri in triangles:
            face = [points[tri[0]],points[tri[1]],points[tri[2]] ]
            self.add_face(face)
